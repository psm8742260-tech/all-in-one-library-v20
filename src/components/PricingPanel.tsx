...
export default function PricingPanel({
  ...
  if (isAdmin) {
    return (
      <div className="bg-orange-50 border-l border-orange-300 text-slate-900 p-4 h-full flex flex-col space-y-4">
        {/* Admin Verified Banner */}
        <div className="bg-gradient-to-r from-emerald-950 to-teal-950 border border-emerald-500/30 rounded-2xl p-4 shadow-lg text-white">
          <div className="flex items-center gap-1.5 mb-1 text-emerald-400 font-bold uppercase tracking-wider text-[9px]">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 fill-current" />
            <span>Admin Verified</span>
          </div>
          <h3 className="text-base font-serif font-black tracking-wide leading-tight mb-0.5">
            అడ్మిన్ నియంత్రణ ప్యానెల్
          </h3>
          <p className="text-[10px] text-teal-300 font-mono font-bold">Unlimited System Access</p>
        </div>

        {/* Access Tier Specs */}
        <div className="bg-orange-100 border border-orange-300 rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-850 uppercase tracking-wider border-b border-orange-200 pb-1.5">
            <Award className="w-4 h-4 text-emerald-500" />
            <span>సిస్టమ్ అధికారాలు (Admin Privileges)</span>
          </div>
          <ul className="text-[11px] text-slate-800 space-y-2 list-none pl-0 font-sans font-medium leading-relaxed">
            <li className="flex items-center gap-2">
              <span className="text-emerald-500 text-sm font-bold">✓</span>
              <span>అపరిమిత ఉచిత రీడింగ్ సదుపాయం</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="text-emerald-500 text-sm font-bold">✓</span>
              <span>ఉచిత పుస్తకాల డౌన్‌లోడ్స్ & ఆడియో ప్లేబ్యాక్</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="text-emerald-500 text-sm font-bold">✓</span>
              <span>ఎటువంటి క్రెడిట్ పరిమితులు వర్తించవు</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="text-emerald-500 text-sm font-bold">✓</span>
              <span>అడ్మిన్ డాష్‌బోర్డ్ పూర్తి నియంత్రణలు</span>
            </li>
          </ul>
        </div>

        {/* Active Admin Account Info */}
        <div className="bg-orange-100 border border-orange-300 rounded-xl p-4 space-y-2.5">
          <div className="text-xs font-bold text-slate-850 uppercase tracking-wider border-b border-orange-200 pb-1.5">
            ఖాతా వివరాలు (Account Info)
          </div>
          <div className="text-xs space-y-2">
            <div>
              <span className="text-slate-500 text-[10px] block font-mono font-bold">EMAIL ADDRESS</span>
              <span className="font-semibold font-mono text-slate-800 break-all">{userEmail || 'psm8742260@gmail.com'}</span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block font-mono font-bold">PHONE NUMBER</span>
              <span className="font-semibold font-mono text-slate-800">8466062260</span>
            </div>
          </div>
        </div>

        {/* AI Engine Connection Indicator */}
        <div className="bg-orange-100 border border-orange-300 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-orange-400" />
            <div>
              <span className="text-[10px] text-slate-500 block uppercase font-mono leading-none mb-1">AI Engine Status</span>
              <span className="text-xs font-semibold text-slate-800">Brahmastra 3.5 Ultra</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 bg-emerald-100/50 text-emerald-800 px-2 py-1 rounded-full text-[10px] font-bold border border-emerald-300">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span>ఆన్‌లైన్ (Live)</span>
          </div>
        </div>
      </div>
    );
